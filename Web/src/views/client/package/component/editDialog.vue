<template>
	<div class="clientPackage-container">
...
<script lang="ts" setup>
	...
	import { addClientPackage, updateClientPackage } from "/src/api/client/package";
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addClientPackage(values);
				} else {
					await updateClientPackage(values);
				}
				closeDialog();
			} else {
				ElMessage({
