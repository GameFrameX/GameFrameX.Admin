<template>
	<div class="clientVersion-container">
...
<script lang="ts" setup>
	...
	import { addClientVersion, updateClientVersion } from "/src/api/client/version";
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addClientVersion(values);
				} else {
					await updateClientVersion(values);
				}
				closeDialog();
			} else {
				ElMessage({
