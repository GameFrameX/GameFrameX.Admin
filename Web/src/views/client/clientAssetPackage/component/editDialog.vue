...
<script lang="ts" setup>
	...
	import { addClientAssetPackage, updateClientAssetPackage } from "/@/api/client/clientAssetPackage";
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addClientAssetPackage(values);
				} else {
					await updateClientAssetPackage(values);
				}
				closeDialog();
			} else {
				ElMessage({
