<template>
	<div class="clientChannel-container">
...
<script lang="ts" setup>
	...
	import { addClientChannel, updateClientChannel } from "/@/api/client/channel";
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addClientChannel(values);
				} else {
					await updateClientChannel(values);
				}
				closeDialog();
			} else {
				ElMessage({
