		...
		<el-dialog v-model="isShowDialog" width="75%" draggable="">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="间隔时间" prop="interval">
              <el-input-number v-model="ruleForm.interval" placeholder="请输入间隔时间" clearable />
...
<script lang="ts" setup>
	...
	import { addGameBannerAnnouncementEntity, updateGameBannerAnnouncementEntity } from "/@/api/game/gameBannerAnnouncementEntity";
		...
		ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
			if (isValid) {
				...
				if (ruleForm.value.id == undefined || ruleForm.value.id == null || ruleForm.value.id == "" || ruleForm.value.id == 0) {
					await addGameBannerAnnouncementEntity(values);
				} else {
					await updateGameBannerAnnouncementEntity(values);
				}
				closeDialog();
			} else {
				ElMessage({
