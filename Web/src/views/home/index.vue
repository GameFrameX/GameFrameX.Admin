    ...
    <el-row :gutter="15" class="home-card-one mb15">
      ...
      <div class="home-card-item flex">
        <div class="flex-margin flex w100" :class="` home-one-animation`">
          <div class="flex-auto">
            <span class="font30">{{ state.homeData.OnlineCount }}</span>
<!--            <span class="ml5 font16" :style="{ color: v.color1 }">{{ v.num2 }}%</span>-->
            <div class="mt10">{{ state.homeData.OnlineCount }}</div>
          </div>
...
import {getOnlineGameHomeEntity} from '/@/api/home/gameHomeEntity';
...
const getOnlineGameHomeEntityHandler = async () => {
  const res = await getOnlineGameHomeEntity({});
  state.homeData.OnlineCount = res.data.result;
};
getOnlineGameHomeEntityHandler();
// 批量设置 echarts resize
